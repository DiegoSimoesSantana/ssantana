...
export default function AnimatedLogo({ className, showSlogan = true, compact = false }: AnimatedLogoProps) {
  ...
  return (
    <span className={clsx('inline-flex flex-col leading-none', className)} aria-label="Santana">
      <span className={clsx('font-black tracking-tight text-current', titleClasses)}>Santana</span>
      {showSlogan && !compact && (
